...
import network.GlobalMessageHalder;
...
public class Server extends Thread {
    ...
    public static int TimeDissConnect;
    public static int backupTime;
    ...
    private void update() {
        try {
            ...
            // final Map map =
            // MapServer.getMapServer(Server.mapBoss75[Util.nextInt(Server.mapBoss75.length)]);
            // if (map != null) {
            ...
            // final Map map =
            // MapServer.getMapServer(Server.mapBoss65[Util.nextInt(Server.mapBoss65.length)]);
            // if (map != null) {
            // Map.liveBoss(map, (byte) Util.nextInt(15, 29), (byte) 0);
            // textchat += addMapNameChat(numBoss++, map.template.mapName);
            // }
            ...
            // for (byte j = 0; j < Util.nextInt(1, 2); ++j) {
            // final Map map =
            // MapServer.getMapServer(Server.mapBoss55[Util.nextInt(Server.mapBoss55.length)]);
            // if (map != null) {
            // Map.liveBoss(map, (byte) Util.nextInt(15, 29), (byte) 0);
            // textchat += addMapNameChat(numBoss++, map.template.mapName);
            // }
            ...
            // for (byte j = 0; j < Util.nextInt(1, 2); ++j) {
            // final Map map =
            // MapServer.getMapServer(Server.mapBoss45[Util.nextInt(Server.mapBoss45.length)]);
            // if (map != null) {
            // Map.liveBoss(map, (byte) Util.nextInt(15, 29), (byte) 0);
            // textchat += addMapNameChat(numBoss++, map.template.mapName);
            // }
            ...
            // if (itemStands != null && itemStands.timeStart + itemStands.timeEnd <=
            ...
            // Service.AlertMessage(player, Text.get(0, 234), String.format(Text.get(0,
            // 235), wait.item.template.name));
            // ItemWait.arrWait.remove(l);
            // ItemWait.flush();
            // } else if (wait.type == 1) {
            // player.upCoinLock(wait.coinLock, (byte) 2);
            // Service.AlertMessage(player, Text.get(0, 234), String.format(Text.get(0,
            // 237), Util.getFormatNumber(wait.coinLock)));
            // ItemWait.arrWait.remove(l);
            // ItemWait.flush();
            // } else if (wait.type == 2) {
            // player.upCoin(wait.coin, (byte) 1);
            // Service.AlertMessage(player, Text.get(0, 234), String.format(Text.get(0,
            // 238), Util.getFormatNumber(wait.coin)));
            // ItemWait.arrWait.remove(l);
            // ItemWait.flush();
            // } else if (wait.type == 3) {
            // player.upGold(wait.gold, (byte) 2);
            // Service.AlertMessage(player, Text.get(0, 234), String.format(Text.get(0,
            // 239), Util.getFormatNumber(wait.gold)));
            // ItemWait.arrWait.remove(l);
            // ItemWait.flush();
            // } else if (wait.type == 4) {
            // player.getMyChar().updateExp(wait.exp);
            // Service.AlertMessage(player, Text.get(0, 234), String.format(Text.get(0,
            // 240), Util.getFormatNumber(wait.exp)));
            // ItemWait.arrWait.remove(l);
            // ItemWait.flush();
            // }
            // }
            ...
            // } else if (ct.level == 1) {
            // ct.typeWin = (byte) ((ct.totalWhite > ct.totalBlack) ? 0 : ((ct.totalBlack >
            // ct.totalWhite) ? 1 : 2));
            // ct.level = 2;
        ...
        while (this.isrun) {
            try {
                ...
                if (Server.timePrintSize <= 0) {
                    System.out.println("Size_connection=" + " Id_Session=" + Server.id + " timeRun "
                            + Util.getStrTime(System.currentTimeMillis() - Server.l));
                    Server.timePrintSize = 5000;
                } else {
        ...
        try {
            ...
            while (Server.start) {
                try {
                    ...
                    Util.log("Accept socket " + conn + " done size ");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
